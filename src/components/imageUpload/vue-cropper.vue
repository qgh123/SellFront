	...
	methods: {
		...
		startMove (e) {
	      ...
	      this.moveY = (e.clientY ? e.clientY : e.touches[0].clientY) - this.y
	      	...
	      	window.addEventListener('touchend', this.leaveImg, { passive: false })
		      	...
		      	window.addEventListener('touchend', this.cancleTouchScale, { passive: false })
					}
				} else {
					window.addEventListener('mousemove', this.moveImg, { passive: false })
	      	window.addEventListener('mouseup', this.leaveImg, { passive: false })
				}
			} else {
				// 截图ing
				this.cropping = true
				// 绑定截图事件
				window.addEventListener('mousemove', this.createCrop, { passive: false })
				window.addEventListener('mouseup', this.endCrop, { passive: false })
				window.addEventListener('touchmove', this.createCrop, { passive: false })
				window.addEventListener('touchend', this.endCrop, { passive: false })
		...
		moveImg (e) {
			...
			if (e.touches && e.touches.length === 2) {
				...
				window.addEventListener('touchmove', this.touchScale, { passive: false })
				window.addEventListener('touchend', this.cancleTouchScale, { passive: false })
				window.removeEventListener('touchmove', this.moveImg, { passive: false })
				return false
			}
      ...
      var nowY = e.clientY ? e.clientY : e.touches[0].clientY
		...
		// 移动图片结束
		leaveImg (e) {
			window.removeEventListener('mousemove', this.moveImg, { passive: false })
      window.removeEventListener('touchmove', this.moveImg, { passive: false })
      window.removeEventListener('mouseup', this.leaveImg, { passive: false })
      window.removeEventListener('touchend', this.leaveImg, { passive: false })
		},
		// 缩放图片
		scaleImg () {
		...
		cancleScale () {
			if (this.canScale) {
				window.removeEventListener(this.support, this.changeSize, { passive: false })
			}
		},
		// 改变大小函数
		changeSize (e) {
      ...
      var nowY = e.clientY ? e.clientY : e.touches ? e.touches[0].clientY : 0
        ...
        this.startMove(e)
			...
			window.addEventListener('touchmove', this.moveCrop, { passive: false })
			window.addEventListener('touchend', this.leaveCrop, { passive: false })
